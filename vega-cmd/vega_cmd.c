...
static ssize_t vega_sendmsg(int fd, const struct msghdr *mhdr)
{
	ssize_t sndlen;
...
static ssize_t vega_send(int fd, const void *buf, size_t buflen)
{
	ssize_t sndlen;
...
static ssize_t vega_read(int fd, void *buf, size_t buflen)
{
	ssize_t res;
...
static int vega_talk(int cmdfd, int vegafd, const struct msghdr *mhdr,
		      const struct timespec *timeout)
{
	...
	if (fd_addflg(vegafd, O_NONBLOCK, &vegaflg) < 0) {
		err = XSTATUS_INTERNAL;
		goto out;
	}
	...
	for (;;) {
		...
		if (mhdr == NULL && cmdfd < 0 && bytes_wr == bytes_rd
			...
			/* Nothing to read anymore and all data have
			   been sent. Send "exit" so that vega-ppp
		...
		if (mhdr || bytes_wr < bytes_rd) {
			/* Data need to be sent to vega-ppp */
			FD_SET(vegafd, &wfds);
			fdmax = (vegafd > fdmax) ? vegafd : fdmax;
		}
		if (start_read) {
			/* Wait for data from vega-ppp */
			FD_SET(vegafd, &rfds);
			fdmax = (vegafd > fdmax) ? vegafd : fdmax;
		}
		if (cmdfd >= 0 && bytes_rd < sizeof(transbuf)) {
			/* Forward data from input stream to vega-ppp */
			FD_SET(cmdfd, &rfds);
		...
		if (FD_ISSET(vegafd, &wfds)) {
			/* Send request to vega-ppp */
			res = 0;
			if (mhdr) {
				res = vega_sendmsg(vegafd, mhdr);
				if (res < 0) {
					if (res == -1)
						err = XSTATUS_ARGVLEN;
					else
				...
				res = vega_send(vegafd, transbuf + bytes_wr,
						 bytes_rd - bytes_wr);
				if (res < 0) {
					err = XSTATUS_INTERNAL;
					break;
			...
			if (res > 0)
				/* Start reading vega-ppp's response
				   as soon as data have been sent */
				start_read = true;
		}

		if (FD_ISSET(vegafd, &rfds)) {
			/* Read answer from vega-ppp */
			res = vega_read(vegafd, printbuf,
					 sizeof(printbuf) - 1);
			if (res < 0) {
				if (res == -1) {
					fverbf(stderr,
		...
		if (cmdfd >= 0 && FD_ISSET(cmdfd, &rfds)) {
			...
			res = vega_read(cmdfd, transbuf + bytes_rd,
...
static int vega_connect(struct host_params *peer, bool numeric)
{
...
static void print_help(const char *name)
{
	...
	printf("\n\tCOMMAND is the vega-ppp command line to be executed");
...
int main(int argc, char **argv)
{
	...
	rv = vega_talk(inputstream, peer.fd, mh, timeo);
